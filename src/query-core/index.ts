...
import { ReactSafeQueryContext, ReactSafeQueryProvider } from '../components/provider';
...
export const query = <TArgs, TResult>(
  ...
  const useQuery = (
    ...
    const { queryClient } = useContext(ReactSafeQueryContext);

    return useTanStackQuery<TResult, unknown>({
      ...(options || {}),
      queryKey: [_key, args],
      queryFn: async () => {
        try {
...
export const mutation = <TArgs, TResult>(
  ...
  const useMutation = <TContext = unknown>(options?: MutationOptions<TResult, TArgs, TContext>) => {
    ...
    return useTanStackMutation<TResult, unknown, TArgs, TContext>({
      ...options,
...
export const createClient = <T>(router: T) => {

  return {
    ...router,
    Provider: ReactSafeQueryProvider,
  }
}
